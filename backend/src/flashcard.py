...
class FlashcardDB:
    ...
    def insert_flashcard(
        self, front, back, title, creator, set_name, timestamp, marked
    ):
        self.cursor.execute(
            """
            INSERT INTO flashcards (front, back, title, creator, set_name, timestamp, marked)
            VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    def update_flashcard(
        self, card_id, front, back, title, creator, set_name, timestamp, marked
    ):
        self.cursor.execute(
            """
    UPDATE flashcards
    ...
    WHERE id = ?
""",
            (front, back, title, creator, set_name, timestamp, marked, card_id),
        )
